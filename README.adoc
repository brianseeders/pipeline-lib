...
<<_code_passivecontainer_code>>, <<_code_interactivecontainer_code>>, and
<<_code_agentcontainer_code>> instead of using `containerTemplate`
directly. This is because the `containerTemplate` wrappers provided by
this library all share the same `workingDir`, which makes them work nicely
together.
...
NOTE: `inDockerAgent` is a derivative of <<_code_inpod_code>>, so
everything that applies to `inPod` also applies to `inDockerAgent`.
...
NOTE: `inRubyBuildAgent` is a derivative of <<_code_inpod_code>>, so
everything that applies to `inPod` also applies to `inRubyBuildAgent`.
